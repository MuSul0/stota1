...
const Galerie = () => {
  ...
  const images = media.filter(item => item.type === 'image' && item.page_context !== 'startseite' && item.page_context !== 'leistungen' && item.page_context !== 'ueber-uns' && item.page_context !== 'empfehlungsprogramm' && item.page_context !== 'kontakt');
    ...
    if (error) {
      return (
        <div className="text-center py-20 text-red-600">
          Fehler beim Laden der Medien: {error}
        </div>
      );
    }
      ...
      return (
        <div className="text-center py-20">
          ...
          <p className="text-gray-500 mt-2">
            Besuchen Sie den Admin-Bereich, um die ersten Bilder und Videos hochzuladen.
          </p>
        </div>
      );
    }
              ...
              <Card key={item.id} className="overflow-hidden group">
                 <a href={item.url} target="_blank" rel="noopener noreferrer" className="block">
                  <CardContent className="p-0 flex flex-col items-center justify-center h-60 bg-gray-900 text-white">
                      ...
                      <p className="text-center font-semibold px-2">{item.title}</p>
                  </CardContent>
                </a>
              </Card>
            ))}
